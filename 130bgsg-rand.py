...
def baby_step_giant_step_random(public_key_hex, min_value, max_value, curve=SECP256k1, num_random_baby_steps=1000):
    ...
    # Define m como a raiz quadrada inteira do intervalo, mais um
    range_size = max_value - min_value + 1
    m = isqrt(range_size) + 1
    
    # Computa os baby steps e armazena-os em um dicionário
    baby_steps = {}
    tested_private_keys = set()  # Conjunto para armazenar as chaves privadas já testadas
    for _ in range(num_random_baby_steps):
        k = random.randint(min_value, max_value)
        if k in tested_private_keys:
            continue  # Pular se a chave privada já foi testada
        point = k * G
